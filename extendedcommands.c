...
if ( language== 1 )
    ...
    sprintf(buf, "从内置SD卡中选择刷机包", primary_path);
...
if ( language== 1 )
        ...
        sprintf(buf, "从外置SD卡中选择刷机包", extra_paths[i]);
...
if (!strcmp(path,"/storage/sdcard0"))
	 sprintf(buf, "备份到内置SD卡", path);
else
	 sprintf(buf, "备份到外置SD卡", path);
#endif
...
#ifndef sd
if (!strcmp(path,"/storage/sdcard0"))
	 sprintf(buf, "[普通]从内置SD卡中选择备份", path);
else
	 sprintf(buf, "[普通]从外置SD卡中选择备份", path);
#endif
...
#ifndef sd
if (!strcmp(path,"/storage/sdcard0"))
	 sprintf(buf, "删除内置SD卡中的备份文件", path);
else
	 sprintf(buf, "删除外置SD卡中的备份文件", path);
#endif
...
#ifndef sd
if (!strcmp(path,"/storage/sdcard0"))
	 sprintf(buf, "[高级]从内置SD卡中选择备份", path);
else
	 sprintf(buf, "[高级]从外置SD卡中选择备份", path);
#endif
...
void show_power_menu() //add by mandfx
{

    static char* headers[] = {  "mandfx",
                                "",
                                NULL
    };
    ...
    for (;;)
    {
      ...
      /*      case 0:
		...
		ui_print("ROOT权限获取中...\n");
		 ...
		 __system("cp /sbin/su.apk /system/app/su.apk");
		 __system("chmod 775 /system/app/su.apk");
		 __system("cp /sbin/install-recovery.sh /system/etc/install-recovery.sh");
		 __system("chmod 775 /system/etc/install-recovery.sh");
		 __system("cp /sbin/.installed_su_daemon /system/etc/.installed_su_daemon");
		 __system("chmod 664 /system/etc/.installed_su_daemon");
		 __system("cp /sbin/su /system/xbin/su");
         __system("chmod 775 /system/xbin/su");
         ui_print("以获取ROOT权限...\n");
		...
		case 2:
         ensure_path_mounted("/system");
		 __system("rm -r /system/app/su.apk");
		 __system("rm -r /system/etc/install-recovery.sh");
		 __system("rm -r /system/etc/.installed_su_daemon");
         __system("rm -r /system/xbin/su");
         ui_print("以取消ROOT权限...\n");
         ensure_path_unmounted("/system");
		break;
		...
		case 3:
         ensure_path_mounted("/data");
		 __system("cat /data/misc/wifi/wpa_supplicant.conf");
		 __system("cp /data/misc/wifi/wpa_supplicant.conf /sdcard/wifi.txt");
         ui_print("由于过多显示不全以复制到SD卡wifi.txt文件打开即可查看...\n");
         ensure_path_unmounted("/data");
		break;
